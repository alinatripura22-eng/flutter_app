...
router.post('/create', async (req, res) => {
  try {
    ...
    res.status(201).json({
      success: true,
      message: 'Business ad created successfully. Pending admin approval.',
      ad: ad
    });
  } catch (error) {
    res.status(500).json({
...
router.post('/serve', async (req, res) => {
  try {
    ...
    const now = new Date();
    const activeAds = Array.from(adsStorage.values()).filter(ad => {
      return ad.status === 'active' &&
             ad.placements.includes(placement) &&
             new Date(ad.schedule.startDate) <= now &&
             new Date(ad.schedule.endDate) >= now &&
             ad.budget.spent < ad.budget.total;
    });
    
    const eligibleAds = activeAds.filter(ad => 
      ad.shouldShow(userLocation || {}, userAge || 25, userGender || 'all', userInterests || [])
    );
...
router.get('/admin/pending', async (req, res) => {
  try {
    const ads = Array.from(adsStorage.values())
      .filter(ad => ad.status === 'pending_review')
...
router.delete('/:adId', async (req, res) => {
  try {
    ...
    adsStorage.delete(req.params.adId);
